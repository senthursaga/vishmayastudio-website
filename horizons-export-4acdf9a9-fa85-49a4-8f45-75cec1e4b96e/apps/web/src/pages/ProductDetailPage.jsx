      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 glass-card p-8">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            <h1 className="text-4xl font-display text-white mb-2">{product.title}</h1>
            <p className="text-lg text-gray-300 mb-4">{product.subtitle}</p>

            <div className="flex items-baseline gap-3 mb-6">
              <span className="text-4xl font-display gold-text">{price}</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.variants.map(variant => (
                    <Button
                      ...
                      className={`transition-all ${selectedVariant?.id === variant.id ? 'bg-primary text-primary-foreground border-primary' : 'border-white/20 text-white hover:bg-white/10'}`}
                    >
                      {variant.title}
                    </Button>
                  ))}
            ...
            <div className="mt-auto">
              <Button onClick={handleAddToCart} size="lg" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-semibold py-3 text-lg disabled:opacity-50 disabled:cursor-not-allowed" disabled={!canAddToCart || !product.purchasable}>
